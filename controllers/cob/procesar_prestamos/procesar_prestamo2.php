...
if (!is_numeric($tasa_interes)) {
    header('Location: ../../../resources/views/zonas/2-baja_california/supervisor/creditos/prestamos.php?mensaje=La tasa de interés no es válida.');
...
if ($conexion->query($sql) === TRUE) {
    ...
    header('Location: ../../../resources/views/zonas/2-baja_california/supervisor/creditos/crudPrestamos.php?mensaje=Solicitud de préstamo realizada con éxito');
    exit;
} else {
    // Redirigir al usuario a crudprestamo.php con un mensaje de error
    header('Location: ../../../resources/views/zonas/2-baja_california/supervisor/creditos/crudPrestamos.php?mensaje=Error al solicitar el préstamo: ' . $conexion->error);
    exit;
}
